...
### v2.2.2-Han（2026-07-12）— 音频优化与头像同步

**音频**
- 🔊 **扬声器播放改为媒体音道**：`USAGE_VOICE_COMMUNICATION` → `USAGE_MEDIA`，声音从扬声器输出，音量键控制媒体音量
- 🎚️ **智能限幅防爆音**：混音和增益两段均采用峰值检测 + 等比缩放（`peak > 30000` 自动比例压缩），多人同时大声说话不再硬切爆音

**头像**
- 🖼️ **实时头像同步**：新增 `knownAvatarIds` 追踪每个用户的 `avatarId`，服务端改头像后立即发现变化并清除缓存重新下载
- 💾 **去掉磁盘头像缓存**：`AvatarCache.loadAvatar` 不再读写磁盘文件，每次从服务器实时下载，确保头像始终最新

**Bug 修复**
- 🔇 **快速切换 PTT/VA 不再锁死麦克风**：捕获停止时自动重启
- 📱 **主界面与悬浮窗静音状态同步**：`isMicMuted` 改为直接引用 `audioBridge.isMuted`，共用同一个 StateFlow
